...
describe('CultureForm - 실제 API 연동', () => {
  ...
  beforeEach(async () => {
    ...
    const initResponse = await fetch(
      'http://localhost:3000/api/onboarding/init',
      {
        method: 'POST',
  ...
  test('1. 4개 카테고리 평가 시스템', async () => {
    ...
    // 4개 카테고리 확인
    expect(
      screen.getAllByText(/Unit Culture.*Teamwork/)[0]
    ).toBeInTheDocument();
    expect(
      screen.getAllByText(/Benefits.*Compensation/)[0]
    ).toBeInTheDocument();
  ...
  test('2. 실제 API 연동: Culture → Account 이동', async () => {
    ...
    const response = await fetch(
      'http://localhost:3000/api/onboarding/culture',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    const progressResponse = await fetch(
      `http://localhost:3000/api/onboarding/progress/${tempUserId}`
    );
